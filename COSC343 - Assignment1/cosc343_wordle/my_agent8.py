    ...
    def AgentFunction(self, percepts):
      ...
      """
        ...
        if guess_counter == 0:
            ...
            #        num = word_to_letter_indices(i, self.letters)
            #        num = num[0]
            #        num_index = (letter_indexes.index(num))
            #        print("num index is", num_index)
            #        #print(num_index)
             #       string_words = ' '.join(self.possible_words)
              #      words_to_keep = re.findall(rf'\w*{i}\w*', string_words)
               #     for word in words_to_keep:
               #         if word[num_index] == i:
               #             words_to_keep.remove(word)
               #     self.possible_words = words_to_keep
                if states == 0:
                    i2 = self.letters[indexes]
                    num2 = word_to_letter_indices(i2, self.letters)
                    num2 = num2[0]
                    num_index2 = (letter_indexes.index(num2))
                    for word in self.possible_words:
                        #print ("num index",word[num_index])
                        #print ("i",i)
                        if word[num_index2] == i2:
